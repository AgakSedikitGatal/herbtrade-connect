...
const SellerWithdraw = () => {
  ...
  const handleWithdraw = (orderId: string) => {
    toast.success(`Withdrawal request for ${orderId} has been processed!`);
  };

  const handleRefund = (orderId: string) => {
    toast.info(`Refund for ${orderId} has been initiated.`);
  };
  ...
  const filteredOrders = withdrawData.filter(order =>
